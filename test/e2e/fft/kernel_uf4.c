  ...
  for (i = 0; i < NSTAGES; ++i) {
    for (j = 0; j < groupsPerStage; ++j) {
      ...
      for (k = 0; k < buttersPerGroup; k += 4) {
        ...
        temp_real = Wr * data_real[base_hi+k+2] - Wi * data_imag[base_hi+k+2];
        temp_imag = Wi * data_real[base_hi+k+2] + Wr * data_imag[base_hi+k+2];
        data_real[base_hi+k+2] = data_real[base_lo+k+2] - temp_real;
        data_real[base_lo+k+2] += temp_real;
        data_imag[base_hi+k+2] = data_imag[base_lo+k+2] - temp_imag;
        data_imag[base_lo+k+2] += temp_imag;

        temp_real = Wr * data_real[base_hi+k+3] - Wi * data_imag[base_hi+k+3];
        temp_imag = Wi * data_real[base_hi+k+3] + Wr * data_imag[base_hi+k+3];
        data_real[base_hi+k+3] = data_real[base_lo+k+3] - temp_real;
        data_real[base_lo+k+3] += temp_real;
        data_imag[base_hi+k+3] = data_imag[base_lo+k+3] - temp_imag;
        data_imag[base_lo+k+3] += temp_imag;
      }
    }
    groupsPerStage *= 2;
    buttersPerGroup /= 2;
