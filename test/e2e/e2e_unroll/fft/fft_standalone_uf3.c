/* fft standalone: single butterfly stage, BPGROUP=128, manually unrolled x3 */
#define BPGROUP 128
...
void kernel(int *data_real, int *data_imag, int Wr, int Wi, long offset)
{
  ...
  for (; k < BPGROUP; k++) {
    int tr = Wr * data_real[offset + BPGROUP + k] -
             Wi * data_imag[offset + BPGROUP + k];
    int ti = Wi * data_real[offset + BPGROUP + k] +
             Wr * data_imag[offset + BPGROUP + k];
    data_real[offset + BPGROUP + k] = data_real[offset + k] - tr;
    data_real[offset + k]          += tr;
    data_imag[offset + BPGROUP + k] = data_imag[offset + k] - ti;
    data_imag[offset + k]          += ti;
  }
}
