// Compiles the parameterized deterministic histogram kernel to MLIR, then lowers it via Neura.
// RUN: clang++ -S -emit-llvm -O3 -fno-vectorize -fno-unroll-loops -std=c++17 -DDATA_LEN=32 -DBUCKET_LEN=8 -o %t-kernel-full.ll %S/../../benchmark/histogram/histogram_scaled_int.cpp
