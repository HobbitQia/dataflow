#!/bin/bash
# Run baseline with large loop bounds (axpy kN=1024, fir NTAPS=1024, gemv N=512,
# histogram DATA_LEN=1024, relu/bicg LARGE_DATASET, etc.) so that unrolling
...
LOG_FILE="$SCRIPT_DIR/baseline_large_results.log"
TIMEOUT=600
...
# fft: NSTAGES=8, NPOINTS=256 — already large enough (butterflies don't collapse)
...
# relu: use LARGE_DATASET (NI=1000, NJ=1100, NK=1200) instead of SMALL_DATASET
...
# spmv: nnz=100 is a runtime variable, not constant-folded; OK as-is
...
get_source() {
    ...
    if [ "$factor" -eq 1 ]; then
        ...
        # Fallback: check for kernel_uf<factor>.<ext> in kernel_dir (uses -DNTAPS etc.)
...
for factor in "${UNROLL_FACTORS[@]}"; do
    ...
    for kernel in "${KERNELS[@]}"; do
        ...
        OUTPUT="$KERNEL_DIR/kernel-large-mapped-uf${factor}.mlir"
        ALL_LOG="$KERNEL_DIR/baseline_large_uf${factor}.log"
