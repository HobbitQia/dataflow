...
run_test() {
  ...
  if [ ! -f "$in_mlir" ]; then
    echo "${kernel} uf=${uf} ops=${max_ops} area=${max_area}: NO_INPUT"
    return
  fi
  ...
  if [ $ret -eq 124 ]; then
    ...
    echo "${kernel} uf=${uf} ops=${max_ops} area=${max_area}: MAP_ERR"
  else
    local ii rec res
...
KERNELS=(axpy fir hist gemv latnrm spmv gemm)
UFS=(1 2 4)
AREA_VALS=(50000 10000 5000 2000)
OPS_VALS=(20 10 5 3 2)

for k in "${KERNELS[@]}"; do
  echo "### $k"
  for uf in "${UFS[@]}"; do
    for area in "${AREA_VALS[@]}"; do
      for ops in "${OPS_VALS[@]}"; do
        run_test "$k" "$uf" "$ops" "$area"
      done
    done
  done
  echo ""
