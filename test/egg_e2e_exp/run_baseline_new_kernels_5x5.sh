#!/usr/bin/env bash
# Baseline mapping for new kernels: dtw, mvt, conv, latnrm, gemm_v2
# uf = 1 / 2 / 4   (dtw/mvt also have uf8)
set -uo pipefail

NEURA_OPT=/mnt/public/qjj/dataflow/build/tools/mlir-neura-opt/mlir-neura-opt
ARCH=/mnt/public/qjj/dataflow/test/egg_e2e_exp/architecture_all_tiles_mem_5x5.yaml
E2E=/mnt/public/qjj/dataflow/test/egg_e2e_exp
LOG=$E2E/baseline_new_kernels_5x5_results.log
...
run_mapping() {
  ...
  timeout 120 $NEURA_OPT $PIPELINE "$in_mlir" -o "$out_mlir" >/dev/null 2>/dev/null
}
...
# DTW
declare -A DTW_SRC
DTW_SRC[1]="$E2E/dtw/dtw_standalone_uf1.c"
DTW_SRC[2]="$E2E/dtw/dtw_standalone_uf2.c"
DTW_SRC[4]="$E2E/dtw/dtw_standalone_uf4.c"
DTW_SRC[8]="$E2E/dtw/dtw_standalone_uf8.c"

echo "### DTW (SIZE=512, inner x-loop, squared distance)"
for uf in 1 2 4; do
  base="/tmp/bl_5x5_dtw_uf${uf}"
  compile_to_mlir "${DTW_SRC[$uf]}" "$C_FLAGS" "${base}_llvm.mlir" ".*kernel.*" 2>/dev/null \
...
# MVT
declare -A MVT_SRC
MVT_SRC[1]="$E2E/mvt/mvt_standalone_uf1.c"
MVT_SRC[2]="$E2E/mvt/mvt_standalone_uf2.c"
MVT_SRC[4]="$E2E/mvt/mvt_standalone_uf4.c"
MVT_SRC[8]="$E2E/mvt/mvt_standalone_uf8.c"

echo "### MVT (N=256, inner i-loop)"
for uf in 1 2 4; do
  base="/tmp/bl_5x5_mvt_uf${uf}"
  compile_to_mlir "${MVT_SRC[$uf]}" "$C_FLAGS" "${base}_llvm.mlir" ".*kernel.*" 2>/dev/null \
...
echo "### CONV (1D, N=1024, K=7, inner k-loop)"
for uf in 1 2 4; do
  base="/tmp/bl_5x5_conv_uf${uf}"
  compile_to_mlir "${CONV_SRC[$uf]}" "$C_FLAGS" "${base}_llvm.mlir" ".*kernel.*" 2>/dev/null \
...
# -------------------------------------------------------
# LATNRM (layer normalization)
declare -A LATNRM_SRC
LATNRM_SRC[1]="$E2E/latnrm/latnrm_standalone_uf1.c"
LATNRM_SRC[2]="$E2E/latnrm/latnrm_standalone_uf2.c"
LATNRM_SRC[4]="$E2E/latnrm/latnrm_standalone_uf4.c"

echo "### LATNRM (N=512, D=64, inner j-loop)"
for uf in 1 2 4; do
  base="/tmp/bl_5x5_latnrm_uf${uf}"
  compile_to_mlir "${LATNRM_SRC[$uf]}" "$C_FLAGS" "${base}_llvm.mlir" ".*kernel.*" 2>/dev/null \
...
# -------------------------------------------------------
# GEMM_V2 (merged i,k outer loop, j-loop as kernel)
declare -A GEMMV2_SRC
GEMMV2_SRC[1]="$E2E/gemm_v2/kernel_pragma_uf1.c"
GEMMV2_SRC[2]="$E2E/gemm_v2/kernel_pragma_uf2.c"
GEMMV2_SRC[4]="$E2E/gemm_v2/kernel_pragma_uf4.c"

echo "### GEMM_V2 (merged i,k, NI=NJ=NK=64, j-loop)"
for uf in 1 2 4; do
  base="/tmp/bl_5x5_gemmv2_uf${uf}"
  compile_to_mlir "${GEMMV2_SRC[$uf]}" "$C_FLAGS" "${base}_llvm.mlir" ".*kernel.*" 2>/dev/null \
