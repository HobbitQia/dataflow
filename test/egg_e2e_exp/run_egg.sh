#!/bin/bash
# Run egg-based mapping with architecture_all_tiles_mem.yaml (all tiles can read/write memory)
# Uses egg equality saturation (--egg-process-neura) with ppa_spec.yaml before mapping.
# Results saved to egg_results.log
...
LOG_FILE="$SCRIPT_DIR/egg_results.log"
TIMEOUT=600
...
run_mapping_egg() {
    ...
    # Pipeline with egg: ctrl-to-data-flow first, then egg fusion, then insert-data-mov, then map
...
extract_metrics() {
    ...
    # compiled_ii, rec_mii, res_mii are stored as MLIR attributes in the output file
    ...
    # fused_op counts as 1 node; ops inside its region are excluded.
...
for factor in "${UNROLL_FACTORS[@]}"; do
    ...
    for kernel in "${KERNELS[@]}"; do
        ...
        OUTPUT="$KERNEL_DIR/kernel-egg-uf${factor}.mlir"
        ALL_LOG="$KERNEL_DIR/egg_uf${factor}.log"
