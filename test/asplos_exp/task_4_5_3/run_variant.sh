#!/bin/bash
# Task 4.5.3: Run taskfusion pipeline for one app with current binary.
# Usage: run_variant.sh <app> <variant_name> [min_support] [max_iter]
set -e
...
echo "=== Running $VARIANT for $APP ==="

# Step 1: Lower with task fusion
bash "$E2E_DIR/scripts/lower_affine_to_neura.sh" \
  "$SINGLE_FUNC" "$FUSED_DIR/fused_neura.mlir" true "$ARCH_SPEC" 1
...
if [ -f "$MDIR/kernel_mapped.mlir" ]; then
  ...
  echo "MAPPING INCOMPLETE: $success ok, $fail failed"
fi
