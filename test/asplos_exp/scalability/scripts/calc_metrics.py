...
def extract_utilization(mapped_file, num_tiles=16):
    if not os.path.isfile(mapped_file):
        return {}
    with open(mapped_file) as f:
        text = f.read()
    ...
    for idx in range(len(func_positions) - 1):
        ...
        total_slots = num_tiles * ii_vals[0] if ii_vals else 1
...
def compute_baseline_area_per_tile():
...
def process_method(app, method, mapped_file, hw_config_path, baseline_area, baseline_app_latency, sharing, rows, num_tiles, uf_label=""):
...
def process_app_at_path(app, app_dir, baseline_area, rows, num_tiles, uf_label=""):
    baseline_mapped = os.path.join(app_dir, "baseline", "kernel_mapped.mlir")
    baseline_latency = process_method(
        app, "baseline", baseline_mapped, None, baseline_area, 0, "-", rows, num_tiles, uf_label
    )
    for r in rows:
    ...
    for method, dir_name in [("opfusion", "opfusion"), ("taskfusion", "taskfusion")]:
        ...
        for sharing in ["exclusive", "inclusive"]:
            ...
            process_method(
                ...
                baseline_latency, sharing, rows, num_tiles, uf_label
            )
...
def parse_args(args):
    ...
    uf_arg = None
    for arg in args:
        m = re.match(r'--rows=(\d+)', arg)
        if m:
            rows = int(m.group(1))
        m = re.match(r'--cols=(\d+)', arg)
        if m:
            cols = int(m.group(1))
        m = re.match(r'--uf=(.+)', arg)
        if m:
            val = m.group(1)
            uf_arg = "all" if val == "all" else int(val)
    return rows, cols, uf_arg


def main():
    base_dir = sys.argv[1] if len(sys.argv) > 1 else "."
    rows, cols, uf_arg = parse_args(sys.argv[2:])
    num_tiles = rows * cols

    area_spec_path = os.path.join(base_dir, "area_spec.yaml")
    if not os.path.isfile(area_spec_path):
        parent = os.path.dirname(base_dir)
        area_spec_path = os.path.join(parent, "area_spec.yaml")
    load_area_spec(area_spec_path)
    init_baseline_fu_counts()

    baseline_area_per_tile = compute_baseline_area_per_tile()
    baseline_area = baseline_area_per_tile * num_tiles
    rows_list = []
    ...
    for app in ["resnet", "llama", "harris", "lstm"]:
        ...
        if uf_arg is None:
            process_app_at_path(app, app_dir, baseline_area, rows_list, num_tiles)
        elif uf_arg == "all":
            uf_dirs = sorted(glob.glob(os.path.join(app_dir, "uf*")))
            for d in uf_dirs:
                if os.path.isdir(d):
                    m = re.match(r'uf(\d+)', os.path.basename(d))
                    if m:
                        uf = int(m.group(1))
                        process_app_at_path(app, d, baseline_area, rows_list, num_tiles, uf_label=str(uf))
        else:
            uf_path = os.path.join(app_dir, f"uf{uf_arg}")
            process_app_at_path(app, uf_path, baseline_area, rows_list, num_tiles, uf_label=str(uf_arg))
    ...
    print(f"\nCGRA size: {rows}x{cols} ({num_tiles} tiles)")
