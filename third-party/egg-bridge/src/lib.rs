...
fn get_node_op_name(node: &SimpleExprLang) -> &'static str {
    match node {
        ...
        SimpleExprLang::Add(_) => "+",
        SimpleExprLang::Sub(_) => "-",
        SimpleExprLang::Mul(_) => "*",
        SimpleExprLang::Div(_) => "/",
...
/// Run equality saturation and extract with simple cycle-aware cost
///
/// This is a simpler version that just adds extra cost for ctrl_mov nodes
#[no_mangle]
pub extern "C" fn egg_run_saturation_simple_cycle(
    expr_str: *const c_char,
    rules_str: *const c_char,
    config: EggConfig,
) -> EggResult {
    ...
    // Use simple cycle-aware extraction (CycleExtractor)
    let extractor = CycleExtractor::new(&runner.egraph, is_back_edge_node);
    let (_, best_expr) = extractor.find_best(runner.roots[0]);
